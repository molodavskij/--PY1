...
def do_replace(element: str, old_symbols: list, new_symbols: list):
    for index in range(len(old_symbols)):
        element = element.replace(old_symbols[index], new_symbols[index])
    return element
...
def to_csv_file(filename, headers, rows, delimiter=",", new_line="\n"):
    ...
    for item in material:
        f.write(do_replace(str(item), ["[", "]", " ", "'", ","], [""]*4 + [delimiter]) + new_line)
    f.close()
